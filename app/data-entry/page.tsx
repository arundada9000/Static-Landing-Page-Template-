...
function generateTS(products: FormProduct[], currency: string, categories: string[], domain: string, passcode: string): string {
  ...
  const productBlocks = products
    .map((p, i) => {
      ...
      lines.push(`    shortDescription: "${escape(p.shortDescription)}",`);
      lines.push(`    longDescription: "${escape(p.longDescription)}",`);
  ...
  return `// Generated by SajiloDigital Data Entry Tool
// Sent by: ${passcode}
// Origin: ${domain}
...
export const CURRENCY = "${currency.trim()}";
...
export default function DataEntryPage() {
  ...
  // ── Persist ──────────────────────────────────────────────────────────────
  useEffect(() => {
    // Helper: map a ShopProduct → FormProduct
    const toFormProduct = (prod: typeof allProducts[number]): FormProduct => ({
      ...emptyProduct(),
      ...prod,
      uid: makeUid(),
    ...
    try {
      ...
      if (saved) {
        ...
        const savedList: FormProduct[] = p && p.length > 0 ? p : [];

        // Merge: keep saved products, append any new ones from allProducts not yet in saved list
        const savedIds = new Set(savedList.map((x: FormProduct) => x.id));
        const newFromSource = allProducts
          .filter(prod => !savedIds.has(prod.id))
          .map(toFormProduct);

        setProducts([...savedList, ...newFromSource]);

        if (newFromSource.length > 0) {
          toast(`${newFromSource.length} new product${newFromSource.length > 1 ? "s" : ""} synced from allProducts.ts`, { duration: 5000 });
        }
        ...
        // Fresh start — load everything from allProducts
        setProducts(allProducts.map(toFormProduct) as unknown as FormProduct[]);
      }
    } catch { /* ignore */ }
  }, []);
  ...
  const clearAllProducts = () => {
    if (!window.confirm(`Are you sure you want to remove all ${products.length} products? This cannot be undone.`)) return;
    setProducts([]);
    setEditingUid(null);
  ...
  const saveProduct = () => {
    ...
    setCurrent(emptyProduct());
    showToast(editingUid ? "Product updated!" : "Product added to list!", "success");
  };
  ...
  const executeExport = (action: "copy" | "download" | "email" | "whatsapp") => {
    ...
    if (action === "copy") {
      ...
      showToast("allProducts.ts downloaded!", "success");
    } else if (action === "email") {
      // Step 1: Auto-download the .ts file so user can attach it
      const blob = new Blob([generatedCode], { type: "text/plain" });
      ...
      // Step 2: Also copy full code to clipboard as backup
      navigator.clipboard.writeText(generatedCode).catch(() => {});

      // Step 3: Open mailto with a short body (full code is too large for mailto URLs)
      const subject = `Product Catalog — ${products.length} Products [${domain}]`;
      const body =
        `Hello SajiloDigital,\n\n` +
        `Verified Passcode: ${passcode}\n` +
        `Origin: ${domain}\n` +
        `Products: ${products.length}\n\n` +
        `I have attached the allProducts.ts file (auto-downloaded to my computer).\n` +
        `Please replace the existing allProducts.ts in the project with the attached file.\n\n` +
        `The full code is also copied to my clipboard as a backup.\n\n` +
        `Thanks,\n${domain}`;
      window.open(
      ...
      toast.success("File downloaded! Attach 'allProducts.ts' to the email that just opened.", { duration: 6000 });

    } else if (action === "whatsapp") {
      const phone = agencyWhatsapp.replace(/[^0-9]/g, "");

      // Step 1: Copy full code to clipboard so user can paste it in WhatsApp
      navigator.clipboard.writeText(generatedCode).catch(() => {});

      // Step 2: Also auto-download the file as backup
      const blob = new Blob([generatedCode], { type: "text/plain" });
      const blobUrl = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = blobUrl;
      ...
      // Step 3: Open WhatsApp with a short intro (wa.me links have ~2000 char limits)
      const summary = products
        .map((p, i) => `${i + 1}. ${p.name} — ${currency}${parseFloat(p.price || "0").toLocaleString()} (${p.category})`)
        .join("\n");
      const msg =
        `*Product Catalog Update* 📦\n` +
        `From: ${domain} | Passcode: ${passcode}\n\n` +
        `*${products.length} Products:*\n${summary}\n\n` +
        `✅ The full allProducts.ts code is ready.\n` +
        `I will paste it in the next message (copied to clipboard).`;
      window.open(`https://wa.me/${phone}?text=${encodeURIComponent(msg)}`, "_blank");

      toast.success(
        "File downloaded & code copied! Paste it in WhatsApp after sending this intro message.",
        { duration: 8000 }
      );
    }
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "var(--color-surface-alt)" }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <button
              ...
              title={showPreview ? "Hide preview" : "Preview product cards"}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 border border-white/10 text-white text-xs font-bold transition-all"
            >
      ...
      {/* Toast handled by sonner <Toaster> in layout.tsx */}
              ...
              <div className="flex items-center gap-2">
                ...
                {products.length > 0 && (
                  <button
                    ...
                    className="text-[10px] font-bold text-rose-400 hover:text-rose-600 hover:bg-rose-50 px-1.5 py-0.5 rounded-md transition-colors"
                  >
                    Clear all
                  </button>
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center gap-3 pt-4">
                    <button
                      ...
                      onClick={() => setCurrent((p) => ({ ...p, active: !p.active }))}
                    ...
                    >
                      <span
                        ...
                        style={{ transform: current.active ? "translateX(18px)" : "translateX(2px)" }}
                      />
                    </button>
